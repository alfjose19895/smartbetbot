...
export function Footer() {
  ...
  if (isLandingPage) {
    badgeText = language === "en" ? "Direct Support & Sales" : "Asesoría Directa & Ventas";
    title =
      language === "en"
  ...
  return (
    <footer className="border-t border-slate-200 bg-slate-50 py-10 text-slate-600 transition-colors dark:border-slate-800/80 dark:bg-slate-950 dark:text-slate-400">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 space-y-6">
        {/* Support / Sales Callout Card */}
        <div className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 text-center shadow-md dark:border-slate-800 dark:bg-slate-900/90">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-3.5 py-1 text-xs font-black text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 border border-emerald-500/30">
            <span>💬</span>
            <span>{badgeText}</span>
          </span>

          <h3 className="mt-3 text-base sm:text-lg font-black text-slate-900 dark:text-white max-w-xl mx-auto leading-snug">
            {title}
          </h3>

          <p className="mt-1.5 text-xs sm:text-sm text-slate-600 dark:text-slate-400 max-w-lg mx-auto leading-relaxed">
            {subtitle}
          </p>
          ...
          <div className="mt-5 flex justify-center">
            <a
              ...
              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#25D366] px-6 py-3 text-xs sm:text-sm font-black !text-white shadow-lg shadow-emerald-500/25 transition-all hover:bg-[#20bd5a] hover:scale-105 active:scale-95 cursor-pointer"
            >
              <svg
              ...
              <span className="!text-white font-black">{btnText}</span>
